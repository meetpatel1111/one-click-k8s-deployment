resource "kubernetes_deployment" "nginx" {
  ...
  spec {
    ...
    template {
      ...
      spec {
        container { name="nginx"; image="nginx:latest"; ports{container_port=80} }
      }
    }
  }
}
...
resource "kubernetes_deployment" "nodejs" {
  ...
  spec {
    ...
    template {
      ...
      spec {
        container { name="nodejs-app"; image=var.nodejs_docker_image; ports{container_port=3000} }
      }
    }
  }
}
...
resource "kubernetes_deployment" "k8sgpt" {
  metadata { name="k8sgpt"; namespace="k8sgpt" }
  spec {
    ...
    template {
      ...
      spec {
        container { name="k8sgpt"; image="ghcr.io/k8sgpt-ai/k8sgpt:latest"; ports{container_port=80} }
      }
    }
  }
}
